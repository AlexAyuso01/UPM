  ...
  public void run() {
    while (true) {
      ...
      ConcIO.printfnl(
        "Ingreso en %s: %d",
        c, v);
      try { sleep(g.positivo(100)); } catch (InterruptedException exc) { }
    }
  }
}
...
class Ordenante extends Thread {
  ...
  public void run() {
    ...
    while (true) {
      ...
      try { sleep(g.positivo(600)); } catch (InterruptedException exc) { }
    }
  }
}
...
class Consultor extends Thread {
  ...
  public void run() {
    ...
    while (true) {
      ...
      try { sleep(g.positivo(200)); } catch (InterruptedException exc) { }
    }
  }
}
